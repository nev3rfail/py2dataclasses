...
def recursive_repr(fillvalue='...'):
    ...
    def decorating_function(user_function):
        ...
        wrapper.__qualname__ = getattr(user_function, '__qualname__', None)
...
class Repr:
    ...
    def _join(self, pieces, level):
        ...
        if isinstance(indent, int):
            if indent < 0:
                raise ValueError(
                    'Repr.indent cannot be negative int (was %r)' % (indent,)
                )
            indent *= ' '
        try:
            ...
            raise TypeError(
                'Repr.indent must be a str, int or None, not %s' % type(indent)
            )
        return sep.join(('', ) + tuple(pieces) + ('', ))[1:-len(indent) or None]
    ...
    def repr_str(self, x, level):
        s = _orig_repr(x[:self.maxstring])
        if len(s) > self.maxstring:
            i = max(0, (self.maxstring-3)//2)
            j = max(0, self.maxstring-3-i)
            s = _orig_repr(x[:i] + x[len(x)-j:])
    ...
    def repr_int(self, x, level):
        try:
            ...
            if 'sys.set_int_max_str_digits()' not in str(exc):
                raise
            ...
            max_digits = g_MAX_DIGITS
            return ('<%s instance with roughly %d '
                    'digits (limit at %d) at 0x%x>' %
                    (x.__class__.__name__, k, max_digits, id(x)))
