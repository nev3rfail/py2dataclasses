    ...
    def _make_annotate_function(__class__, method_name, annotation_fields, return_type):
        ...
        def __annotate__(format):
            ...
            if format in (Format.VALUE, Format.FORWARDREF, Format.STRING):
                ...
                # Try to get annotations from the class's __annotate__ method if it exists
                # But only if it was NOT generated by dataclasses itself (respect user customization)
                class_annotate = getattr(__class__, '__annotate__', None)
                if (class_annotate is not None and callable(class_annotate) and
                    not getattr(class_annotate, '__generated_by_dataclasses__', False)):
                    try:
                        cls_annotations = class_annotate(format)
                    except Exception:
                        pass

                # If we didn't get annotations from custom __annotate__, try __annotations__ directly
                if not cls_annotations:
                    for base in reversed(__class__.__mro__):
                        # Get annotations directly from __dict__ to avoid recursion
                        # Use __dict__ to bypass any __annotate__ descriptors
...
def _process_class(cls, init, repr, eq, order, unsafe_hash, frozen,
    ...
    if repr:
        ...
        if flds:
            ...
            body_line = '  return "{0}({1})".format(self.__class__.__name__, {0})'.format(
                repr_fmt, field_refs
            )
            body = [body_line]
