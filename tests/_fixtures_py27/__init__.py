...
class TestDocString(unittest.TestCase):
    ...
    def test_docstring_one_field_with_default_none(self):
        ...
        # Check that the docstring shows the Union type, accepting None as a default
        self.assertIn('x:', C.__doc__)
        self.assertIn('None', C.__doc__)
    ...
    def test_docstring_with_no_signature(self):
        ...
        # Python 2.7 doesn't support metaclass Meta(type): syntax, so we use __metaclass__
        class Meta(object):
            __call__ = dict
    ...
    def test_docstring_with_unsolvable_forward_ref_in_init(self):
        ...
        # Python 2.7 can't use exec in the same way, so we simulate with string types
        @dataclass
        class C(object):
            __annotations__ = {'x': 'X', 'num': int}
            def __init__(self, x, num):
                self.x = x
                self.num = num

        # In py27 with string annotations, the docstring should still work
        self.assertIn('x:', C.__doc__)
        self.assertIn('num:', C.__doc__)

    def test_init_false_forwardref(self):
        # Test init=False with forward references
        @dataclass
        class C(object):
            __annotations__ = {'x': 'ForwardRef', 'y': int}
            y = field(int)
            x = field('ForwardRef', init=False, default=None)
    ...
    def test_slots_true_init_false(self):
        # Test slots=True with init=False fields
